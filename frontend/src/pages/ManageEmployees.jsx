...
function StepDot({ n, current, label }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 w-24 flex-shrink-0 relative">
      ...
      <span
        className={`text-[9px] font-black uppercase tracking-[0.14em] text-center hidden sm:block ${active ? "text-slate-800" : "text-slate-400"}`}
      >
        {label}
      </span>
    </div>
...
export default function ManageEmployees({ setActiveTab }) {
  ...
  const validateStep = (step) => {
    ...
    if (step === 3) {
      ...
      if (isManager && !formData.branchId) {
        errs.branchId =
          "A branch manager must be assigned to a specific branch location.";
      }
    }
    setModalErrors(errs);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!editingEmployee && !finalPassword) {
        const cleanName = formData.firstName
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "");
      ...
      if (editingEmployee) {
        ...
        setEmployees((prev) =>
          prev.map((e) =>
            e._id === editingEmployee._id ? { ...e, ...payload } : e,
          ),
        );
        ...
        if (wasPasswordAutoGenerated) {
          toast.success(
            <div className="flex w-full items-start gap-3 px-3.5 py-2.5">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[0.8rem] border border-emerald-250 bg-emerald-50 text-emerald-700 text-xs font-black shadow-sm">
                ✓
              </div>
              <div className="min-w-0 flex-1 pr-8 text-left">
                ...
                <div className="mt-0.5 text-[0.78rem] font-semibold text-emerald-950/85">
                  Successfully added {formData.firstName} as{" "}
                  {formData.role === "inventory_manager" ||
                  formData.role?.endsWith("_inventory_manager")
                    ? "Inventory Manager"
                    : formData.role === "manager" ||
                        formData.role?.endsWith("_manager")
                      ? "Branch Manager"
                      : "Employee"}
                  .
                  <button
  ...
  const handleToggleActive = async (emp) => {
    ...
    setEmployees((prev) =>
      prev.map((e) =>
        e._id === emp._id ? { ...e, isActive: updatedStatus } : e,
      ),
    );
    try {
      await updateEmployee(emp._id, { isActive: updatedStatus });
      toast.success(
        `${emp.firstName} is now ${updatedStatus ? "Active" : "Deactivated"}`,
      );
      fetchData();
    } catch (err) {
      // Revert state
  ...
  const handleDeactivate = async (empId) => {
    if (
      !window.confirm(
        "Are you sure you want to deactivate this staff member? They will lose system access immediately.",
      )
    ) {
      return;
    }
  ...
  const getRoleLabel = (role) => {
    ...
    if (roleStr === "inventory_manager") return "Inventory Manager";
    if (roleStr === "warehouse_inventory_manager")
      return "Warehouse Inventory Manager";
    if (roleStr === "franchise_inventory_manager")
      return "Franchise Inventory Manager";
    if (roleStr === "depot_inventory_manager") return "Depot Inventory Manager";
    if (roleStr === "store_inventory_manager") return "Store Inventory Manager";
    if (roleStr.endsWith("_manager")) {
      const prefix = roleStr.split("_")[0];
      return prefix.charAt(0).toUpperCase() + prefix.slice(1) + " Manager";
    }
    if (roleStr === "employee" || roleStr === "staff" || roleStr === "cashier")
      return "Staff / Cashier";
    if (
      roleStr.endsWith("_employee") ||
      roleStr.endsWith("_staff") ||
      roleStr.endsWith("_cashier")
    ) {
      ...
      return (
        prefix.charAt(0).toUpperCase() + prefix.slice(1) + " " + formattedSuffix
      );
    }
  ...
  const activeManagers = employees.filter(
    (e) =>
      (e.role === "manager" ||
        (e.role?.endsWith("_manager") && !e.role?.includes("inventory")) ||
        (e.role === "owner" && e.branchId)) &&
      e.isActive,
  ).length;
  ...
  const allocatedStaffCount = React.useMemo(() => {
    return branches.reduce(
      (sum, b) => sum + 1 + (Number(b.employee_count) || 0),
      0,
    );
  }, [branches]);
  ...
  const showMismatchWarning =
    allocatedStaffCount > totalStaff && totalStaff > 0;
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    const isEmpManager =
      emp.role === "manager" ||
      emp.role?.endsWith("_manager") ||
      (emp.role === "owner" && emp.branchId);
    const matchesRole =
      roleFilter === "all" ||
        ...
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 md:px-12 py-3.5">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[8px] font-black uppercase tracking-[0.22em] text-emerald-700 hidden md:block leading-none mb-1">
                Enterprise Resources
              </span>
              <h3 className="text-sm md:text-lg font-black leading-tight truncate">
      ...
      <main className="px-6 md:px-12 py-6 max-w-7xl mx-auto space-y-6">
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <div className="rounded-2xl border bg-white border-slate-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between">
            <div>
              ...
              <span className="block text-3xl font-black mt-1 text-slate-700">
                {totalStaff}
              </span>
            </div>
          ...
          <div className="rounded-2xl border bg-white border-sky-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between bg-sky-50/40">
            <div>
              ...
              <span className="block text-3xl font-black mt-1 text-sky-600">
                {allocatedStaffCount}
              </span>
              <span className="block text-[9px] font-bold text-slate-400 mt-1">
                across {branches.length} branch
          ...
          {/* Unregistered */}
          <div
            className={`rounded-2xl border bg-white p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between ${missingAccountsCount > 0 ? "border-amber-200 bg-amber-50/40" : "border-slate-100"}`}
          >
            <div>
              <span className="block text-[9px] font-black uppercase tracking-wider text-slate-400">
                Unregistered
              ...
              <span
                className={`block text-3xl font-black mt-1 ${missingAccountsCount > 0 ? "text-amber-600" : "text-slate-300"}`}
              >
          ...
          <div className="rounded-2xl border bg-white border-amber-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between bg-amber-50/50">
            <div>
              ...
              <span className="block text-3xl font-black mt-1 text-amber-600">
                {activeManagers}
              </span>
            </div>
            <span className="text-3xl">🎯</span>
          ...
          <div className="rounded-2xl border bg-white border-emerald-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between bg-emerald-50/50">
            <div>
              ...
              <span className="block text-3xl font-black mt-1 text-emerald-600">
                {activeEmployees}
              </span>
            </div>
            <span className="text-3xl">✅</span>
          ...
          <div className="rounded-2xl border bg-white border-rose-100 p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center justify-between bg-rose-50/50">
            <div>
              ...
              <span className="block text-3xl font-black mt-1 text-rose-600">
                {suspendedCount}
              </span>
            </div>
            <span className="text-3xl">🚫</span>
                ...
                <div className="flex gap-4 items-start flex-1">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <h4 className="text-base font-black text-slate-900 leading-snug">
                      {missingAccountsCount} Staff Member
                      {missingAccountsCount !== 1 ? "s" : ""} Cannot Access the
                      System
                    </h4>
                    <p className="text-sm font-semibold text-slate-600 leading-relaxed max-w-2xl">
                      Your branch configuration allocates{" "}
                      <span className="font-black text-slate-900">
                        {allocatedStaffCount} team positions
                      </span>
                      , but only{" "}
                      <span className="font-black text-slate-900">
                        {totalStaff} login account{totalStaff !== 1 ? "s" : ""}
                      </span>{" "}
                      exist in the system. These {missingAccountsCount}{" "}
                      unregistered member{missingAccountsCount !== 1 ? "s" : ""}{" "}
                      will be locked out until credentials are created.
                    </p>
                  </div>
                </div>
                ...
                <div className="shrink-0 flex flex-col items-center lg:items-end gap-3 w-full lg:w-auto">
                  ...
                  <div className="w-full lg:w-[240px]">
                    <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">
                      ...
                      <span>
                        {totalStaff}/{allocatedStaffCount}
                      </span>
                    </div>
                    <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.round((totalStaff / Math.max(1, allocatedStaffCount)) * 100))}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <section className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {/* Filter Group */}
            <div
              className={`grid ${isOwner ? "grid-cols-2" : "grid-cols-1"} md:flex md:items-center gap-3 w-full md:w-auto`}
            >
              {/* Filter by Role */}
                ...
                <div className="flex flex-col items-center gap-5 max-w-lg mx-auto">
                  ...
                  <div className="space-y-2">
                    ...
                    <h4 className="text-xl font-black text-slate-900 tracking-tight leading-snug">
                      Staff Positions Allocated,
                      <br />
                      But No Accounts Exist Yet
                    </h4>
                    <p className="text-sm font-semibold text-slate-600 leading-relaxed">
                      Your branches have{" "}
                      <span className="font-black text-amber-700">
                        {allocatedStaffCount} declared team positions
                      </span>
                      , but zero login credentials have been created. Your staff
                      will not be able to access Inventra until you register
                      their accounts.
                    </p>
                  </div>
                ...
                <div className="flex flex-col items-center gap-5 max-w-md mx-auto">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-semibold text-slate-500 leading-relaxed">
                      Register your first team member to grant secure access to
                      Inventra. Managers get full branch control, while
                      employees are scoped to the Billing POS.
                    </p>
                  </div>
                  <button
                    onClick={openAddModal}
                ...
                <tbody className="divide-y divide-slate-100 font-semibold text-slate-800">
                  {filteredEmployees.map((emp) => (
                    ...
                    >
                              ...
                              emp.role === "owner"
                                ...
                                : emp.role === "manager" ||
                                  ...
                                  : emp.role === "inventory_manager" ||
                                      emp.role?.endsWith("_inventory_manager")
                                    ? "bg-blue-500 shadow-sm shadow-blue-100"
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider ${
                            emp.role === "owner"
                      ...
                      <td className="px-5 py-4 text-center">
                        {emp.role === "owner" ? (
                          ...
                          <button
                            ...
                            className={`inline-flex rounded-full border px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider cursor-pointer transition-all hover:scale-105 ${
                              emp.isActive
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="relative custom-phone-prefix-container">
                          ...
                          <div className="flex relative">
                            <div className="absolute left-0 top-0 bottom-0 flex items-center z-10">
                              <button
                                type="button"
                                onClick={() =>
                                  setPhonePrefixOpen(!phonePrefixOpen)
                                }
                              ...
                              >
                                <span>
                                  {COUNTRIES.find(
                                    (c) =>
                                      c.code === formData.phone_country_code,
                                  )?.flag || "🇮🇳"}
                                </span>
                                ...
                                <svg
                                  className={`w-2.5 h-2.5 text-slate-500 transition-transform ${phonePrefixOpen ? "rotate-180" : ""}`}
                                  fill="none"
                                  stroke="currentColor"
                                  strokeWidth="2.5"
                                  viewBox="0 0 24 24"
                              ...
                              {phonePrefixOpen && (
                                <div className="absolute left-0 top-full mt-1.5 w-44 bg-white border border-slate-200 rounded-xl shadow-2xl p-1.5 z-50 animate-fade-in flex flex-col gap-0.5 max-h-48 overflow-y-auto">
                                  {COUNTRIES.map((c) => {
                                    const isSelected =
                                      formData.phone_country_code === c.code;
                                    return (
                                      <button
                                        key={c.name}
                                        type="button"
                          ...
                          <p className="text-[10px] font-semibold text-slate-400 mt-2 leading-relaxed">
                            Used for account recovery and branch contact.
                          ...
                          <div>
                            <Label>
                              {editingEmployee
                                ...
                                : "Access Password (Leave blank to generate automatically)"}
                            </Label>
                            <div className="relative">
                              <input
                                ...
                                placeholder={
                                  editingEmployee
                                    ? "••••••••"
                                    : "Leave blank to auto-generate or min 6 chars"
                                }
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 gap-3 mt-1.5">
                            {formData.role === "owner" ? (
                              <div className="p-4 rounded-2xl border-2 border-slate-900 bg-slate-900 text-white shadow-sm flex items-center gap-4">
                                ...
                                <div>
                                  ...
                                  <span className="text-[9.5px] font-semibold mt-0.5 leading-snug text-slate-300 block">
                                    Primary business owner. Full administrative
                                    rights.
                                  </span>
                                </div>
                              </div>
                              ...
                              (() => {
                                const branchHasActiveManager = (branchId) =>
                                  !!branchId &&
                                  employees.some(
                                    (e) =>
                                      (e.role === "manager" ||
                                        (e.role?.endsWith("_manager") &&
                                          !e.role?.includes("inventory"))) &&
                                      e.branchId === branchId &&
                                      e.isActive,
                                  );

                                const selectedBranchHasManager =
                                  branchHasActiveManager(formData.branchId);
                                const availableBranchForManager = branches.find(
                                  (branch) =>
                                    !branchHasActiveManager(branch.branch_id),
                                );
                                const canShowManagerRole =
                                  isOwner &&
                                  (formData.branchId
                                    ? !selectedBranchHasManager
                                    : !!availableBranchForManager);
                                ...
                                const roles = [
                                  {
                                    ...
                                    icon: "🧾",
                                    title: "Staff / Cashier",
                                    desc: "Locks permissions to branch POS checkout & billing operations.",
                                  },
                                  {
                                    value: "inventory_manager",
                                    icon: "📋",
                                    title: "Inventory Manager (Staff)",
                                    desc: "Specializes in inventory control. Manages stock levels, items, and expiry tracking.",
                                  },
                                ];

                                // Only show Branch Manager if at least one branch still needs one
                                if (canShowManagerRole) {
                                  roles.push({
                                    value: "manager",
                                    icon: "🎯",
                                    title: "Branch Manager",
                                    desc: "Manage branch operations, staff, and inventory scope. ONE per branch only.",
                                  });
                                }
                                ...
                                return roles.map((r) => {
                                  const isSelected =
                                    r.value === "manager"
                                      ? formData.role === "manager" ||
                                        (formData.role?.endsWith("_manager") &&
                                          !formData.role?.includes("inventory"))
                                      : r.value === "inventory_manager"
                                        ? formData.role ===
                                            "inventory_manager" ||
                                          formData.role?.endsWith(
                                            "_inventory_manager",
                                          )
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-4 ${
                                        isSelected
                                          ? "border-emerald-600 bg-emerald-50/70 text-emerald-900 shadow-sm cursor-pointer"
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <span className="text-[9.5px] font-semibold mt-0.5 leading-snug text-slate-400 block">
                                          {r.desc}
                                        </span>
                                      </div>
                                    </button>
                    ...
                    (() => {
                      ...
                      if (!isOwner) {
                        const myBranch = branches.find(
                          (b) => b.branch_id === userSession?.user?.branchId,
                        );
                        return (
                          <div className="space-y-4">
                            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                              <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider">
                                Local Branch Assignment
                              </h4>
                              <span className="text-[10px] font-bold text-slate-400">
                                Locked to your branch location
                              </span>
                            </div>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setFormData((p) => ({
                                      ...
                                      role:
                                        (p.role === "manager" ||
                                          (p.role?.endsWith("_manager") &&
                                            !p.role?.includes("inventory"))) &&
                                        employees.some(
                                          (e) =>
                                            (e.role === "manager" ||
                                              (e.role?.endsWith("_manager") &&
                  ...
                  {modalStep === 4 &&
                    (() => {
                      const assignedBranch = branches.find(
                        (b) => b.branch_id === formData.branchId,
                      );
                      const isManager =
                        formData.role === "manager" ||
                        formData.role?.endsWith("_manager");
                      ...
                      return (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                            ...
                            <span className="text-[10px] font-bold text-slate-400">
                              All set? Double-check before saving.
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="rounded-2xl border border-slate-100 bg-white p-4 flex items-center gap-3">
                                <div
                                  className={`h-12 w-12 shrink-0 rounded-xl flex items-center justify-center font-black text-base text-white shadow-md ${isManager ? "bg-gradient-to-br from-amber-400 to-orange-500" : "bg-gradient-to-br from-emerald-400 to-emerald-600"}`}
                                >
                                ...
                                <span
                                  className={`shrink-0 inline-flex rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider border ${isManager ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-emerald-50 border-emerald-200 text-emerald-800"}`}
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="flex flex-wrap gap-1.5 mt-2">
                                        <span className="inline-flex rounded-full bg-slate-100 border border-slate-200 px-2 py-0.5 text-[9px] font-black text-slate-600">
                                          Code:{" "}
                                          {assignedBranch.branch_code || "BRN"}
                                        </span>
                                        <span className="inline-flex rounded-full bg-slate-100 border border-slate-200 px-2 py-0.5 text-[9px] font-black text-slate-600">
                                          {assignedBranch.branch_type ||
                                            "Store"}
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] font-semibold text-slate-400 mt-0.5">
                                        Not locked to any specific branch
                                      </p>
                                    </div>
                                  </div>
                                )}
                            ...
                            {/* RIGHT: Role & Access */}
                            <div
                              className={`rounded-2xl border p-4 flex flex-col ${isManager ? "border-amber-100 bg-amber-50/40" : "border-emerald-100 bg-emerald-50/30"}`}
                            >
                              ...
                              <div className="space-y-2 flex-1">
                                {(isManager ? managerPerms : employeePerms).map(
                                  (perm) => (
                                    <div
                                      key={perm}
                                    ...
                                    >
                                      <span
                                        className={`w-4 h-4 shrink-0 rounded-full flex items-center justify-center text-[9px] font-black text-white ${isManager ? "bg-amber-400" : "bg-emerald-400"}`}
                                      >
                                        ✓
                                      </span>
