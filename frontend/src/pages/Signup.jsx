...
export default function Signup({ setActiveTab }) {
  ...
  const [classification, setClassification] = React.useState(draftState.classification);
  ...
  const validateStep = (s) => {
    ...
    if (s === 1) {
      ...
      if (!form.email) {
        ...
        const re = /^\S+@\S+\.\S+$/;
        if (!re.test(form.email)) e.email = 'Enter a valid work email (e.g. you@company.com).';
      }
      if (!form.password) {
        e.password = 'Choose a password.';
      } else {
        const pw = form.password;
        if (pw.length < 8) e.password = 'Weak password. Use at least 8 characters.';
      }
      if (!form.agree) e.agree = 'You must accept the Terms of Service and Privacy Policy.';
    }
    if (s === 2) {
      // business step validation minimal
      if (!form.company) e.company = 'Company name is required for onboarding.';
      // numeric fields optional but normalize
    }
    return Object.keys(e).length ? e : null;
  };

  const classifyBusiness = ({ inventorySize, transactionsLast30d, branches, employees }) => {
    // normalize values
  ...
  const computeMLFeatures = (f) => {
    ...
    if (classifierSchema && classifierSchema.features) {
      ...
      if (s.complexity && s.complexity.breaks) breaks.complexity = s.complexity.breaks;
    }
    ...
    const rawSales = Number((f.transactionsLast30d || '').toString().replace(/[,\s]/g, '')) || 0;
    const avgTicket = Number(f.avgTicket || 500) || 500; // default ₹500 if not provided
    const estTransactions = Math.max(0, Math.round(rawSales / Math.max(1, avgTicket)));
    ...
    if (step === 3) {
      ...
      fetchMLClassification(features).then((ml) => {
        if (ml && ml.classification) {
          ...
          setForm(prev => ({ ...prev, mlConfidence: ml.confidence, signalQuality: ml.signalQuality }));
        } else {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (json && json.detail) {
          ...
          if (json.detail.toLowerCase().includes('email')) {
            setErrors({ email: json.detail });
            throw new Error('Email already in use');
          }
          setErrors({ submit: json.detail });
  ...
  const checkDatabaseStatus = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/v1/auth/debug/signup-status?email=${encodeURIComponent(form.email)}`);
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json.detail?.message || json.detail || 'Unable to verify database record.');
      setDbCheck(json);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-[#F8FAFC] font-sans lg:h-screen">
      ...
      <div className="hidden lg:flex lg:w-[48%] 2xl:w-1/2 bg-[#0F172A] text-white p-8 lg:pt-6 lg:px-12 xl:pt-8 xl:px-16 2xl:pt-10 2xl:px-20 flex flex-col justify-between relative overflow-hidden lg:h-full">
        {/* Neon blurred background nodes for premium ambient glow */}
        <div className="absolute w-[400px] h-[400px] bg-[#0EA5E9]/10 blur-[100px] rounded-full top-[-100px] left-[-100px] pointer-events-none"></div>
        <div className="absolute w-[350px] h-[350px] bg-slate-800/20 blur-[80px] rounded-full bottom-[-100px] right-[-100px] pointer-events-none"></div>
        ...
        <div className="relative z-10 max-w-md xl:max-w-lg my-4 md:my-auto flex flex-col gap-6 text-left">
          <div>
            <span className="text-[#0EA5E9] text-xs font-black uppercase tracking-widest block mb-3">Enterprise Intelligence</span>
            <h1 className="text-3xl md:text-4xl lg:text-[2.7rem] xl:text-5xl 2xl:text-5xl font-black leading-[1.06] text-white tracking-tight mb-4">
              The future of retail is <span className="text-[#0EA5E9]">predictive.</span>
            </h1>
            <p className="text-[14px] md:text-[15.5px] text-slate-400 leading-relaxed font-medium">
              Join the intelligence network. Transform your inventory management, sales forecasting, and branch operations with adaptive AI designed for scale.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3.5 mt-1.5">
            ...
            <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 xl:p-5 hover:border-[#0EA5E9]/30 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(14,165,233,0.03)] cursor-default flex items-start gap-3.5">
              <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center flex-shrink-0 border border-white/10 text-[#0EA5E9]">
              ...
              <div className="flex flex-col gap-1 text-left">
                <span className="font-bold text-[13.5px] text-white">Stock Intelligence</span>
                <span className="text-[11.5px] text-slate-400 font-semibold leading-relaxed">Real-time demand forecasting that reduces manual stock audits and out-of-stocks.</span>
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 xl:p-5 hover:border-[#0EA5E9]/30 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(14,165,233,0.03)] cursor-default flex items-start gap-3.5">
              ...
              <div className="flex flex-col gap-1 text-left">
                <span className="font-bold text-[13.5px] text-white">Customer Trends</span>
                <span className="text-[11.5px] text-slate-400 font-semibold leading-relaxed">Identify high-value purchasing segments and forecast transaction vectors.</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full lg:w-[52%] p-4 sm:p-8 md:p-12 lg:pt-4 lg:px-10 xl:pt-6 xl:px-12 2xl:pt-8 2xl:px-16 pb-6 sm:pb-8 md:pb-10 lg:pb-12 flex items-start justify-start lg:h-full min-h-0 overflow-auto lg:overflow-visible">
        <div className="w-full max-w-sm sm:max-w-md lg:max-w-[760px] xl:max-w-[840px] 2xl:max-w-[920px] origin-top">
          ...
          <div className="flex justify-between items-center mb-3 sm:mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[11px] font-black uppercase tracking-widest text-[#0EA5E9] block leading-none mb-0.5">Platform Signup</span>
                <span className="text-[14px] font-black text-slate-900 block leading-none font-sans">Inventra</span>
              </div>
            </div>
          </div>
          ...
          {!success && (
              ...
              <div className="flex justify-between items-center mb-2.5">
                <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400">Step {step} of 4</span>
                <span className="text-[12.5px] font-bold text-[#0EA5E9]">
                  {step === 1 ? 'Personal Profile' : step === 2 ? 'Business Info' : step === 3 ? 'Business Metrics' : 'Confirm Registration'}
                </span>
              </div>
              <div className="h-1.5 w-full bg-slate-200/60 rounded-full overflow-hidden flex gap-0.5">
                <div className={`h-full flex-1 transition-all duration-500 rounded-l-full ${step >= 1 ? 'bg-[#0EA5E9]' : 'bg-slate-200/60'}`}></div>
                <div className={`h-full flex-1 transition-all duration-500 ${step >= 2 ? 'bg-[#0EA5E9]' : 'bg-slate-200/60'}`}></div>
                <div className={`h-full flex-1 transition-all duration-500 ${step >= 3 ? 'bg-[#0EA5E9]' : 'bg-slate-200/60'}`}></div>
                <div className={`h-full flex-1 transition-all duration-500 rounded-r-full ${step >= 4 ? 'bg-[#0EA5E9]' : 'bg-slate-200/60'}`}></div>
              </div>
            </div>
          )}
          ...
          {success ? (
            <div className="p-7 bg-white border border-emerald-100 rounded-2xl text-center shadow-[0_20px_50px_rgba(16,185,129,0.03)] flex flex-col items-center gap-5">
              ...
              <div className="flex flex-col gap-2">
                <h3 className="text-2xl font-black text-slate-900 leading-tight">Account Created Successfully!</h3>
                <p className="text-[14px] text-slate-500 leading-relaxed font-medium">
                  We've saved your account and business record. You can verify it in the database below.
                </p>
              </div>
              <div className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left space-y-2 text-[13px] font-medium text-slate-600">
                <div><span className="font-bold text-slate-900">Email:</span> {form.email}</div>
                <div><span className="font-bold text-slate-900">Business:</span> {form.company}</div>
                <div><span className="font-bold text-slate-900">Tier:</span> {classification || 'small'}</div>
              </div>
              <button
                type="button"
              ...
              {dbCheck && !dbCheck.error && (
                <div className="w-full rounded-2xl border border-slate-200 bg-white p-4 text-left space-y-2 text-[13px] font-medium text-slate-600">
                  <div><span className="font-bold text-slate-900">User record:</span> {dbCheck.userExists ? 'Found' : 'Missing'}</div>
                  <div><span className="font-bold text-slate-900">Business record:</span> {dbCheck.businessExists ? 'Found' : 'Missing'}</div>
                  {dbCheck.classification && <div><span className="font-bold text-slate-900">Saved tier:</span> {dbCheck.classification}</div>}
                </div>
              )}
              {dbCheck && dbCheck.error && (
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 md:p-6 lg:p-6 xl:p-7 shadow-[0_20px_50px_rgba(0,0,0,0.02)] w-full flex flex-col gap-3 text-left">
              ...
              {step === 1 && (
                <>
                  <div className="flex flex-col gap-1 mb-2">
                    <h2 className="text-xl sm:text-[1.7rem] lg:text-[1.75rem] xl:text-[1.8rem] 2xl:text-[1.8rem] font-black text-slate-950 tracking-tight leading-tight">Setup your account</h2>
                    <p className="text-xs sm:text-sm md:text-[14px] lg:text-sm text-slate-600 font-medium leading-relaxed">Let's start with your contact information to setup your workspace.</p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-slate-500">First Name</label>
                      <input 
                      ...
                      />
                      {errors && errors.firstName && <p className="text-red-500 text-[11px] font-bold mt-0 px-1 leading-tight">{errors.firstName}</p>}
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">Last Name</label>
                      <input 
                        ...
                        className={inputClass} 
                      />
                      {errors && errors.lastName && <p className="text-red-500 text-[11px] font-bold mt-0 px-1 leading-tight">{errors.lastName}</p>}
                    </div>
                  </div>
                  
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">Business Email</label>
                    <input 
                      ...
                      className={inputClass} 
                    />
                    <p className="text-[11px] text-slate-500 mt-1">We'll send setup and verification details to this address.</p>
                    {errors && errors.email && <p className="text-red-500 text-[11px] font-bold mt-0 px-1 leading-tight">{errors.email}</p>}
                  </div>

                  <div className="flex flex-col gap-1 relative">
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">Create a Password</label>
                    <input 
                      ...
                      className={inputClass} 
                    />
                    {errors && errors.password && <p className="text-red-500 text-[11px] font-bold mt-0 px-1 leading-tight">{errors.password}</p>}
                    <span className={`text-[10px] font-bold block mt-1 px-1 leading-normal ${passwordIsStrong ? 'text-emerald-600' : 'text-red-500'}`}>
                      {passwordIsStrong ? 'Good — secure enough to continue.' : 'Use at least 8 characters for security.'}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5 mt-1.5 border-t border-slate-50 pt-3.5">
                    <div className="flex items-start gap-3">
                      ...
                      />
                      <label htmlFor="agree" className="text-[12.5px] text-slate-650 font-bold leading-normal cursor-pointer select-none">
                        I accept the <span className="text-[#0EA5E9] hover:underline">Terms of Service</span> and acknowledge the <span className="text-[#0EA5E9] hover:underline">Privacy Policy</span>.
                      </label>
                    </div>
                    {errors && errors.agree && <p className="text-red-500 text-[11px] font-bold mt-0.5 px-1 leading-tight">{errors.agree}</p>}
                  </div>
                </>
              )}
              ...
              {step === 2 && (
                <>
                  <div className="flex flex-col gap-1 mb-2">
                    <h2 className="text-xl sm:text-[1.7rem] lg:text-[1.75rem] xl:text-[1.8rem] 2xl:text-[1.8rem] font-black text-slate-950 tracking-tight leading-tight">About your business</h2>
                    <p className="text-xs sm:text-sm md:text-[14px] lg:text-sm text-slate-600 font-medium leading-relaxed">Tell us the basics so we can set up your account.</p>
                  </div>

                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">Business or Store Name</label>
                    <input 
                      ...
                      className={inputClass} 
                    />
                    <p className="text-[11px] text-slate-500 mt-1">Enter the name customers know your business by.</p>
                    {errors && errors.company && <p className="text-red-500 text-[11px] font-bold mt-0 px-1 leading-tight">{errors.company}</p>}
                  </div>
              ...
              {step === 3 && (
                <>
                  <div className="flex flex-col gap-1 mb-2">
                    <h2 className="text-xl sm:text-[1.7rem] lg:text-[1.75rem] xl:text-[1.8rem] 2xl:text-[1.8rem] font-black text-slate-950 tracking-tight leading-tight">Tell us about your business</h2>
                    <p className="text-xs sm:text-sm md:text-[14px] lg:text-sm text-slate-600 font-medium leading-relaxed">Just a few simple details so we can set things up for you.</p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1">
                      <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">How many products do you stock?</label>
                      <input 
                        ref={inventoryRef}
                        ...
                        className={inputClass} 
                      />
                      <p className="text-[11px] text-slate-500 mt-1">Roughly how many products or items do you sell?</p>
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">About how many staff do you have?</label>
                      <input 
                        ...
                        className={inputClass} 
                      />
                      <p className="text-[11px] text-slate-500 mt-1">Rough total across all locations is okay.</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1 h-full">
                      <label className="min-h-[2.5rem] text-[11px] font-extrabold uppercase tracking-wider text-slate-500 leading-tight">How many locations do you have?</label>
                      <input 
                        value={form.branches} 
                        ...
                        className={inputClass} 
                      />
                      <p className="text-[11px] text-slate-500 mt-1">Just the total number of places you sell from.</p>
                    </div>
                    <div className="flex flex-col gap-1 h-full">
                      <label className="min-h-[2.5rem] text-[11px] font-extrabold uppercase tracking-wider text-slate-500 leading-tight">Monthly sales</label>
                      <input 
                        value={form.transactionsLast30d} 
                        onChange={(e) => handleChange('transactionsLast30d', e.target.value)} 
                        placeholder="e.g., 5,000 (₹)" 
                        className={inputClass} 
                      />
                      <p className="text-[11px] text-slate-500 mt-1">Enter monthly sales in INR.</p>
                    </div>
                  </div>

                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">What kind of business is this? (optional)</label>
                    <BusinessTypeSelect
                      value={form.businessType}
                      onChange={(v) => handleChange('businessType', v)}
                      className=""
                    />
                    <p className="text-[11px] text-slate-500 mt-1">Pick the closest match. It helps us set things up better.</p>
                  </div>
                </>
              )}
              ...
              {errors && errors.submit && step < 4 && (
                <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl flex items-center gap-2.5 text-rose-600 text-[11px] font-bold leading-normal">
                  <svg className="w-4 h-4 text-rose-500 stroke-[2.5] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {step === 4 && (
                <div className="flex flex-col gap-3">
                  <ConfirmRegistration form={form} classification={classification} />
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 mt-1.5 sm:mt-2 border-t border-slate-50 pt-3 sm:pt-4">
                    ...
                    >
                      <span>{loading ? 'Creating workspace...' : 'Confirm & Launch'}</span>
