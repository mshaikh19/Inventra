...
const getRoleDisplayName = (role) => {
  ...
  if (r === "warehouse_inventory_manager") return "WAREHOUSE INVENTORY MANAGER";
  if (r === "franchise_inventory_manager") return "FRANCHISE INVENTORY MANAGER";
  if (r === "depot_inventory_manager") return "DEPOT INVENTORY MANAGER";
  if (r === "store_inventory_manager") return "STORE INVENTORY MANAGER";
...
const DASHBOARD_CONFIG = {
  small: {
    ...
    blurb:
      "A beginner-friendly workspace for single-location operators who need clarity, speed, and simple controls.",
    accent: "#0284C7",
    accentSoft: "rgba(2, 132, 199, 0.08)",
    shell: "#0284C7",
    summary:
      "Practical control panel for stock, billing, alerts, and day-to-day sales visibility.",
    profile: {
  ...
  medium: {
    ...
    blurb:
      "Operational control for growing businesses that need forecasting, procurement discipline, and performance visibility.",
    accent: "#D97706",
    accentSoft: "rgba(217, 119, 6, 0.08)",
    shell: "#D97706",
    summary:
      "Balanced workspace for procurement, automated reorders, category segments, and reporting.",
    profile: {
  ...
  large: {
    ...
    blurb:
      "Enterprise command center for branch networks, warehouse distribution hubs, and NLP-powered AI insights.",
    accent: "#059669",
    accentSoft: "rgba(5, 150, 105, 0.08)",
    accentBase: "#059669",
    summary:
      "Central command center for regional demand, branch transfers, automated allocations, and NLP insights.",
    profile: {
...
function StepDot({ n, current, label }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 flex-1 relative">
      ...
      <span
        className={`text-[10px] font-black uppercase tracking-[0.14em] hidden sm:block ${active ? "text-slate-800" : "text-slate-400"}`}
      >
        {label}
      </span>
    </div>
...
function StepConnector({ done }) {
  return (
    <div className="flex-1 self-center px-1 mb-4">
      <div
        className={`h-[2.5px] w-full rounded-full transition-colors duration-500 ${done ? "bg-emerald-400" : "bg-slate-200"}`}
      />
    </div>
  );
}
...
function getDynamicLabels(type) {
  const defs = {
    Store: {
      ...
      codePlaceholder: "e.g. STORE-001",
      addressLabel: "Store Address",
      addressPlaceholder: "e.g. 42 MG Road",
      ...
      gstLabel: "GSTIN",
      gstPlaceholder: "e.g. 27AABCU9603R1ZM",
      staffLabel: "Staff Count",
      ...
      gstSectionLabel: "🧾 GSTIN",
      hoursSectionLabel: "⏰ Hours",
    ...
    Warehouse: {
      ...
      addressPlaceholder: "e.g. Plot 5, Industrial Area",
      cityLabel: "Warehouse City",
      cityPlaceholder: "e.g. Pune",
      pincodeLabel: "Warehouse Pincode",
      pincodePlaceholder: "e.g. 411001",
      managerLabel: "Warehouse Manager",
      placeholderName: "e.g. Suresh Patel",
      phoneLabel: "Warehouse Phone",
      phonePlaceholder: "e.g. 98765 43210",
      emailLabel: "Manager Email",
      emailPlaceholder: "manager@example.com",
      gstLabel: "GSTIN",
    ...
    Factory: {
      ...
      namePlaceholder: "e.g. Manufacturing Unit",
      codeLabel: "Factory Code",
      codePlaceholder: "e.g. FAC-001",
      addressLabel: "Factory Address",
      addressPlaceholder: "e.g. 12B Industrial Zone",
      cityLabel: "Factory City",
      cityPlaceholder: "e.g. Chennai",
      pincodeLabel: "Factory Pincode",
      pincodePlaceholder: "e.g. 600001",
      managerLabel: "Factory Manager",
      placeholderName: "e.g. Priya Sharma",
      phoneLabel: "Factory Phone",
      phonePlaceholder: "e.g. 98765 43210",
      emailLabel: "Manager Email",
      emailPlaceholder: "manager@example.com",
      gstLabel: "GSTIN",
    ...
    Outlet: {
      ...
      namePlaceholder: "e.g. Airport Kiosk",
      codeLabel: "Outlet Code",
      codePlaceholder: "e.g. OUT-001",
      addressLabel: "Outlet Address",
      addressPlaceholder: "e.g. Terminal 2",
      cityLabel: "Outlet City",
      cityPlaceholder: "e.g. Delhi",
      pincodeLabel: "Outlet Pincode",
      pincodePlaceholder: "e.g. 110001",
      managerLabel: "Outlet Manager",
      placeholderName: "e.g. Amit Verma",
      phoneLabel: "Outlet Phone",
      phonePlaceholder: "e.g. 98765 43210",
      emailLabel: "Manager Email",
      emailPlaceholder: "manager@example.com",
      gstLabel: "GSTIN",
...
export default function Dashboard({ tier: normalizedTier, setActiveTab }) {
  ...
  const isManager = useMemo(() => {
    ...
    const role = String(userSession?.user?.role || "")
      .trim()
      .toLowerCase();
    ...
    return (
      role === "manager" ||
      (role.endsWith("_manager") && !role.includes("inventory")) ||
      (roles.includes("manager") && !roles.some((r) => r.includes("inventory")))
    );
  }, [isOwner, userSession]);

  const isEmployee = useMemo(() => {
    if (isOwner || isManager) return false;
    // Employees include inventory managers + regular staff/cashiers
    return (
      isEmployeeUser(userSession?.user) ||
      isInventoryManagerUser(userSession?.user)
    );
  ...
  const userBranch = useMemo(() => {
    ...
    return (
      branchesList?.find((b) => b.branch_id === userBranchId) ||
      branchesList?.[0]
    );
  ...
  const employeeQuickActions = useMemo(() => {
    ...
    return getEmployeeQuickActions(
      employeeEnvironment,
      normalizedTier,
      setActiveTab,
    );
  ...
  const visibleTabs = useMemo(() => {
    ...
    if (isEmployee) {
      ...
      // Regular employees based on branch type
    ...
    return [];
  }, [isOwner, isManager, isEmployee, branchType, userSession]);
  ...
  const [showAddBranchModal, setShowAddBranchModal] = useState(() => {
    if (typeof window !== "undefined") {
      return (
        sessionStorage.getItem("inventra_show_add_branch_modal") === "true"
      );
    }
    return false;
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem(
        "inventra_show_add_branch_modal",
        showAddBranchModal ? "true" : "false",
      );
    }
  }, [showAddBranchModal]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem(
        "inventra_add_branch_modal_step",
        modalStep.toString(),
      );
    }
  }, [modalStep]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem(
        "inventra_add_branch_modal_form",
  ...
  const [modalCountryDropdownOpen, setModalCountryDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    if (modalForm.branch_name && !modalForm.branch_code) {
      setModalForm((p) => ({
        ...p,
        branch_code: autoCode(modalForm.branch_name),
      }));
    }
    ...
    if (branchesList && branchesList.length > 0) {
      if (normalizedTier === "small") {
        ...
        getBranchInventory(branchId)
          ...
          .catch((err) => {
            console.warn(
              "Failed to load branch inventory for dashboard overview:",
              err,
            );
          });
      } else {
        ...
        Promise.all(
          branchesList.map((branch) => {
            ...
            return getBranchInventory(branchId)
              ...
              .catch((err) => {
                console.warn(
                  `Failed to fetch inventory for branch ${branch.branch_name}:`,
                  err,
                );
                const cached = loadScopedInventoryProducts(
                  [],
                  branch.branch_name,
                );
                return cached.map((p) => ({
  ...
  const handleDeleteAccount = async () => {
    ...
    if (
      ...
      setDeleteError(
        "Email does not match. Please type your exact account email to confirm.",
      );
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        "http://127.0.0.1:8000/api/v1/auth/delete-account",
        {
          method: "DELETE",
  ...
  const handleRecoverAccount = async () => {
    ...
    try {
      const res = await fetch(
        "http://127.0.0.1:8000/api/v1/auth/recover-account",
        {
          method: "POST",
  ...
  const handleSaveBranch = async () => {
    const payload = {
      ...
      phone: (
        modalForm.phone_country_code +
        " " +
        modalForm.phone_number.trim()
      ).trim(),
    ...
    try {
      ...
      if (modalForm._id) {
        ...
        payload.manager_password =
          `${firstName}_${lastName}@manager`.toLowerCase();
      ...
      toast.success(
        <div className="flex w-full items-center gap-3 px-3.5 py-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[0.8rem] border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-black shadow-sm">
            ✓
          </div>
          <div className="min-w-0 flex-1 pr-8 text-left">
            ...
            <div className="mt-0.5 text-[0.78rem] font-semibold text-emerald-950/80">
              {modalForm.branch_name}{" "}
              {modalForm._id
                ? "successfully updated inside database."
                : "registered in database as " + modalForm.branch_code + "."}
            </div>
          </div>
        </div>,
        {
      ...
      toast.error(
        <div className="flex w-full items-center gap-3 px-3.5 py-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[0.8rem] border border-rose-250 bg-rose-50 text-rose-700 text-xs font-black">
            !
          </div>
          <div className="min-w-0 flex-1 pr-8 text-left">
            <div className="font-heading text-[0.86rem] font-extrabold tracking-[-0.02em] text-rose-700">
              Failed to Save Branch
            </div>
            <div className="mt-0.5 text-[0.78rem] font-semibold text-rose-950/80">
              {err.message || "Unable to communicate with the server."}
            </div>
          </div>
        </div>,
        {
      ...
      toast.error(
        <div className="flex w-full items-center gap-3 px-3.5 py-2.5">
          ...
          <div className="min-w-0 flex-1 pr-8 text-left">
            ...
            <div className="mt-0.5 text-[0.78rem] font-semibold text-rose-950/80">
              {err.message || "Unable to open edit form"}
            </div>
          </div>
        </div>,
        {
  ...
  const handleDeleteBranch = async (branchId) => {
    if (
      !window.confirm(
        "Are you sure you want to permanently deactivate this branch?",
      )
    )
      return;
    setLoading(true);
      ...
      toast.success(
        <div className="flex w-full items-center gap-3 px-3.5 py-2.5">
          ...
          <div className="min-w-0 flex-1 pr-8 text-left">
            ...
            <div className="mt-0.5 text-[0.78rem] font-semibold text-emerald-950/80">
              Branch node deactivated and removed from live system.
            </div>
          </div>
        </div>,
        {
      ...
      toast.error(
        <div className="flex w-full items-center gap-3 px-3.5 py-2.5">
          ...
          <div className="min-w-0 flex-1 pr-8 text-left">
            ...
            <div className="mt-0.5 text-[0.78rem] font-semibold text-rose-950/80">
              {err.message || "Failed to remove branch"}
            </div>
          </div>
        </div>,
        {
  ...
  const handleSaveProfile = async (e) => {
    ...
    if (
      ...
      !emailRegex.test(profileDraft.email.trim())
    ) {
  ...
  const renderInteractiveSeasonalChart = () => {
    ...
    const baseLine =
      mediumRange === "7d" ? salesHistory.map((h) => h.revenue) : [];

    const upperLine = baseLine.map((v) => v + Math.round(v * 0.15));
    const lowerLine = baseLine.map((v) =>
      Math.max(0, v - Math.round(v * 0.15)),
    );
    ...
    const getX = (index) =>
      padding + (index / (dates.length - 1)) * (width - padding * 2);
    const getY = (val) =>
      height -
      padding -
      15 -
      ((val - minVal) / range) * (height - padding - 35);

    const baseCoords = baseLine.map((val, idx) => `${getX(idx)},${getY(val)}`);
    const upperCoords = upperLine.map(
      (val, idx) => `${getX(idx)},${getY(val)}`,
    );
    const lowerCoords = lowerLine.map(
      (val, idx) => `${getX(idx)},${getY(val)}`,
    );
        ...
        <div className="relative overflow-hidden border border-slate-200/60 bg-gradient-to-b from-slate-50 to-white rounded-2xl p-4 shadow-inner">
              ...
              return (
                <g key={idx} className="cursor-pointer group">
                  <circle
                    ...
                    fill="#D97706"
                    opacity="0"
                    className="hover:opacity-20 transition-all duration-205"
                  />
                  <circle
                    cx={cx}
                    cy={cy}
          ...
          {!hasSales ? (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-[3px] flex flex-col items-center justify-center text-center p-6 z-10 border border-slate-100/50">
              <div className="h-10 w-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center shadow-sm text-sm mb-2">
                📈
              </div>
              ...
              <p className="text-xs font-semibold text-slate-500 mt-2 max-w-sm leading-relaxed">
                Holiday-aware profit forecasting and confidence boundaries will
                activate once transactions are recorded in Billing POS.
              </p>
            </div>
          ) : null}
        </div>
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[9px] font-semibold text-slate-500 block mt-1 leading-none">
                        {c.label} • {c.fill}
                      </span>
                    </div>
                  </div>
        ...
        {!hasSales ? (
          <div className="absolute inset-0 bg-white/70 backdrop-blur-[3px] flex flex-col items-center justify-center text-center p-6 z-10 border border-slate-100/50">
            ...
            <p className="text-xs font-semibold text-slate-500 mt-2 max-w-xs leading-relaxed">
              Category sales distribution and segment shares will populate once
              transactions are recorded in Billing POS.
            </p>
          </div>
        ) : null}
      </div>
  ...
  const simulateChatCommand = (promptText) => {
    ...
    if (activeBranches.length === 0 && activeProducts.length === 0) {
      ...
      if (lowStock.length > 0) {
        replyText = `Found ${lowStock.length} low-stock items: ${lowStock
          .slice(0, 3)
          .map((p) => p.name)
          .join(", ")}. Consider restocking soon.`;
      } else {
      ...
      <div className="flex flex-col h-[320px] bg-slate-950 border border-slate-800 rounded-3xl overflow-hidden font-mono text-xs text-left">
        ...
        <div className="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest pl-2">
              INVENTRA CO-PILOT COMMAND
            </span>
          </div>
  ...
  const renderGeographicalCommandMap = () => {
    const branches = {
      Mumbai: {
        ...
        efficiency: "98.2%",
        fleet: "12 Fleet Vehicles",
        stocks: "1,240 units",
        coord: [100, 160],
      },
      Delhi: {
        name: "Delhi Command Center",
        efficiency: "94.6%",
        fleet: "8 Fleet Vehicles",
        stocks: "850 units",
        coord: [120, 50],
      },
      Chennai: {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1.1fr_0.9fr] gap-4 bg-slate-950 border border-slate-800 p-4 rounded-3xl select-none min-h-[320px] text-left">
        ...
        <div className="relative border border-slate-800 bg-slate-950 rounded-2xl overflow-hidden p-2 flex items-center justify-center min-h-[220px]">
          <svg className="w-full h-full max-h-[240px]" viewBox="0 0 300 300">
            ...
            <polygon
              points="120,30 180,40 210,120 180,180 170,250 140,290 120,240 80,180 60,150 90,100"
              fill="none"
              stroke="#1E293B"
              strokeWidth="1.5"
            />
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col justify-between text-left font-sans">
          <div>
            ...
            <h4 className="text-base font-black text-slate-200 mt-1">
              {activeInfo.name}
            </h4>
            <p className="text-[11px] text-slate-400 font-semibold mt-1">
              Multi-branch node evaluating transfer vectors and transport lines.
            </p>
          </div>
          ...
          <div className="rounded-[28px] border border-slate-100 bg-white px-5 py-5 shadow-[0_10px_25px_rgba(15,23,42,0.04)]">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <h2 className="text-lg font-black text-slate-900 mt-0.5 leading-tight">
                  {isEmployee
                    ? employeeEnvironment?.label || "My Branch Desk"
                    : config.label}
                </h2>
              </div>
              <span
                className="inline-flex items-center rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-wider text-white shadow-sm"
                style={{ background: config.shell }}
              >
                {tierBadgeLabel}
              </span>
            </div>
            ...
            {isEmployee && userBranch && (
              <div className="mt-3 rounded-2xl border border-slate-100 bg-slate-50/80 px-3 py-2.5">
                ...
                <div className="mt-1 text-[11px] font-bold text-slate-800 leading-tight">
                  {employeeEnvironment?.icon} {userBranch.branch_name} ·{" "}
                  {userBranch.branch_type}
                </div>
              </div>
            )}
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span
                      className={`grid h-8 w-8 place-items-center rounded-xl text-sm transition-colors ${isActive ? "bg-white/20" : "bg-slate-50 group-hover:bg-slate-100"}`}
                    >
                      {tab.icon}
                    </span>
                  ...
                  <span
                    className={`text-[10px] transition-transform ${isActive ? "translate-x-0 opacity-100" : "translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"}`}
                  >
                    →
                  </span>
                </button>
            ...
            <div className="mt-4 rounded-[28px] border border-slate-100 bg-white p-5 shadow-[0_10px_25px_rgba(15,23,42,0.04)]">
              <details className="group" open>
                <summary className="text-[9px] font-black uppercase tracking-[0.24em] text-slate-500 cursor-pointer list-none flex justify-between items-center hover:text-slate-900 transition-colors">
                  ...
                  <span className="text-[8px] transition-transform duration-300 group-open:rotate-180">
                    ▼
                  </span>
                </summary>
                <div className="mt-3.5 flex flex-wrap gap-2">
                  {tierFeatures.map((f, i) => (
                    <div
                      key={i}
                      className="min-w-0 flex-1 basis-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-[10.5px] font-semibold text-slate-600 leading-snug"
                    >
                      {f}
                    </div>
                  ))}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <span className="block text-xs font-black text-slate-900 group-hover:text-slate-950">
                        {action.label}
                      </span>
          ...
          <div className="mt-4 space-y-3.5">
            <div className="rounded-[24px] border border-slate-100 bg-slate-950 p-4 text-white shadow-[0_12px_25px_rgba(15,23,42,0.12)]">
              ...
              <p className="mt-2 text-[11px] leading-relaxed text-slate-300">
                Low stock and expiry signals are tracked in real time for the
                smallest workflow noise possible.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {QUICK_LINKS.map((link) => (
                <button
                  key={link.label}
                  className="rounded-2xl border border-slate-200 bg-white px-2.5 py-3 text-[10px] font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors"
                >
        ...
        <main className="min-w-0 space-y-6 px-4 sm:px-6 lg:px-8 xl:px-10 py-6 lg:py-8">
          ...
          {activeSection === "overview" && (
            <section className="bg-white border border-slate-200 p-5 md:p-6 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left relative overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
              {/* Glowing spot */}
              <div
                className="absolute w-48 h-48 rounded-full pointer-events-none -right-10 blur-[80px]"
              ...
              <div>
                ...
                <h1 className="text-2xl md:text-3xl font-black text-slate-900 mt-1 leading-tight tracking-tight">
                  {config.strap}
                </h1>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-slate-200 p-5 md:p-6 rounded-3xl text-left shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                    ...
                    <p className="text-xs text-slate-500 font-semibold mt-1 leading-relaxed">
                      High-fidelity intelligence forecast vectors for primary
                      bakery and dairy SKUs, calculated dynamically using local
                      sales frequency parameters.
                    </p>
                  </div>
                      ...
                      [...products]
                        ...
                        .map((product) => {
                          ...
                          return (
                            <div
                              key={product.id}
                              className="relative overflow-hidden bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05)] text-left flex flex-col justify-between"
                            >
                              <div>
                                ...
                                <div className="grid grid-cols-3 gap-2.5 mb-6">
                                  <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2">
                                    ...
                                    <span
                                      className={`text-sm font-black block mt-0.5 ${isLowStock ? "text-rose-600" : "text-slate-800"}`}
                                    >
                                      {product.stock} Units
                                    </span>
                                    <span
                                      className={`text-[8.5px] font-semibold block ${isLowStock ? "text-rose-500" : "text-slate-500"}`}
                                    >
                                      {isLowStock
                                        ? `Below Reorder (${product.reorderLevel})`
                                        : `Healthy (min ${product.reorderLevel})`}
                                    </span>
                                  </div>
                                  <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2">
                                    <span className="text-[8px] font-black uppercase tracking-wider text-slate-400 block">
                                    ...
                                    <span className="text-sm font-black text-slate-800 block mt-0.5">
                                      {hasSales ? `${forecast} Units` : "—"}
                                    </span>
                                    <span
                                      className={`text-[8.5px] font-semibold block ${hasSales ? "text-emerald-600" : "text-slate-400"}`}
                                    >
                                      {growth}
                                    </span>
                                  </div>
                                  <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2">
                                    ...
                                    <span className="text-[8.5px] font-semibold text-slate-500 block">
                                      {hasSales ? "High Certainty" : "Pending"}
                                    </span>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="relative pt-4 pb-2 border-t border-slate-100">
                                ...
                                >
                                  <defs>
                                    ...
                                    >
                                      <stop
                                        ...
                                        stopOpacity={hasSales ? 0.25 : 0.05}
                                      />
                                      <stop
                                        offset="100%"
                                        stopColor={config.accent}
                                  ...
                                  <path
                                    ...
                                    fill="none"
                                    stroke={
                                      hasSales ? config.accent : "#cbd5e1"
                                    }
                                    strokeWidth="3"
                                    strokeDasharray={hasSales ? "none" : "4 4"}
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                  />
                                </svg>
                              ...
                              {!hasSales ? (
                                <div className="absolute inset-0 bg-white/75 backdrop-blur-[3px] rounded-3xl flex flex-col items-center justify-center text-center p-4 z-10 border border-slate-100/50">
                                  ...
                                  <p className="text-xs font-semibold text-slate-500 mt-2 max-w-[240px] leading-relaxed">
                                    Awaiting sales activity to calibrate
                                    predictive models. Record transactions in
                                    Billing POS to generate forecasts.
                                  </p>
                                </div>
                              ) : null}
                            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 text-left">
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h3 className="text-lg font-black text-slate-900 mb-1">
                          Seasonal & Profit Analytics
                        </h3>
                        <p className="text-xs text-slate-500 font-semibold">
                          Dual path Area forecasts with seasonal peaks.
                        </p>
                      </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)] space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-semibold">
                        Category Sales donut segments indicators.
                      </p>
                    </div>
                    <div className="py-2">{renderInteractiveDonutChart()}</div>
                  </div>
                </div>
              )}
              {normalizedTier === "large" &&
                (() => {
                  const networkHasSales = products.some(
                    (p) => (p.sold || 0) > 0,
                  );
                  return (
                      ...
                      <section className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-6">
                          <div>
                            ...
                            <h3 className="text-2xl md:text-3xl font-black text-slate-950 mt-1 leading-tight">
                              Network Demand & Inventory Risk Command
                            </h3>
                            <p className="text-xs md:text-sm text-slate-500 font-semibold mt-2 max-w-3xl leading-relaxed">
                              Multi-branch forecast model combining sales
                              velocity, stock cover, expiry exposure,
                              fulfillment load, and transfer feasibility across
                              regional hubs.
                            </p>
                          </div>
                            ...
                            {[
                              {
                                label: "Total Sales",
                                value:
                                  salesCount > 0 ? String(salesCount) : "—",
                                note:
                                  salesCount > 0 ? "transactions" : "No data",
                                tone:
                                  salesCount > 0
                                    ? "text-emerald-600"
                                    : "text-slate-400",
                              },
                              {
                                ...
                                note: salesRevenue > 0 ? "current" : "No data",
                                tone:
                                  salesRevenue > 0
                                    ? "text-emerald-600"
                                    : "text-slate-400",
                              ...
                              >
                                ...
                                <span className="block text-lg font-black text-slate-950 mt-1">
                                  {metric.value}
                                </span>
                                <span
                                  className={`block text-[10px] font-black uppercase tracking-wider mt-0.5 ${metric.tone}`}
                                >
                                  {metric.note}
                                </span>
                              </div>
                          ...
                          <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-50/70 p-4 md:p-5">
                            ...
                            >
                              <defs>
                                ...
                                >
                                  <stop
                                    offset="0%"
                                    stopColor="#10B981"
                                    stopOpacity={networkHasSales ? 0.25 : 0.05}
                                  />
                                  <stop
                                    offset="100%"
                                    stopColor="#10B981"
                                ...
                                >
                                  <stop
                                    offset="0%"
                                    stopColor="#F59E0B"
                                    stopOpacity={networkHasSales ? 0.18 : 0.02}
                                  />
                                  <stop
                                    offset="100%"
                                    stopColor="#F59E0B"
                              ...
                              {networkHasSales &&
                                (() => {
                                  const b1 =
                                    branchesList[0]?.branch_name ||
                                    "Primary Hub";
                                  const b2 =
                                    branchesList[1]?.branch_name ||
                                    "Secondary Hub";
                                  return [
                            ...
                            {!networkHasSales ? (
                              <div className="absolute inset-0 bg-white/70 backdrop-blur-[3px] flex flex-col items-center justify-center text-center p-6 z-10 border border-slate-100/50">
                                ...
                                <p className="text-xs font-semibold text-slate-500 mt-2 max-w-sm leading-relaxed">
                                  Enterprise command neural charts and network
                                  demand forecasting will activate once sales
                                  are registered in POS.
                                </p>
                              </div>
                            ) : null}
                          </div>
                          ...
                          <div className="space-y-3">
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span
                                      className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-wider ${tone}`}
                                    >
                                      {signal}
                                    </span>
                                  </div>
                            ...
                            {branchesList.length === 0 && (
                              <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-6 text-center text-xs font-semibold text-slate-500">
                                No branches configured. Setup branch networks in
                                Branch Operations.
                              </div>
                            )}
                          </div>
                      ...
                      <section className="grid grid-cols-1 xl:grid-cols-[0.85fr_1.15fr] gap-6">
                        <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
                          ...
                          {products.length > 0 ? (
                            <div className="mt-5 space-y-3">
                              {products
                                ...
                                .map((p, i) => (
                                  ...
                                  >
                                    <span className="h-7 w-7 rounded-full bg-emerald-600 text-white text-xs font-black grid place-items-center shrink-0">
                                      {i + 1}
                                    </span>
                                    <div>
                                      ...
                                      <p className="text-[11px] font-semibold text-slate-500 leading-relaxed mt-0.5">
                                        Current stock: {p.stock}, reorder level:{" "}
                                        {p.reorderLevel}. {p.category} category
                                        needs attention.
                                      </p>
                                    </div>
                                  </div>
                            ...
                            <p className="text-xs font-semibold text-slate-400 text-center py-4">
                              No inventory data available for recommendations.
                            </p>
                          )}
                        </div>
                        ...
                        <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
                          ...
                          <h3 className="text-lg font-black text-slate-900 mt-1">
                            Inventory, Margin & Expiry Analysis
                          </h3>
                          {products.length > 0 ? (
                            ...
                            <p className="text-xs font-semibold text-slate-400 text-center py-4">
                              No inventory data for category analysis.
                            </p>
                          )}
                        </div>
                      </section>
          ...
          {activeSection === "tasks" && (
            <div className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-white p-5 md:p-6 shadow-[0_14px_44px_rgba(15,23,42,0.06)] text-left">
              {/* Header */}
              <div
                className="absolute right-10 top-0 h-1.5 w-24 rounded-b-full"
                style={{ backgroundColor: config.accent }}
              />
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 leading-relaxed mt-2 max-w-xl">
                    {employeeEnvironment?.blurb ||
                      "Inspect your active branch assignments and check off completed duties."}
                  </p>
                  {userBranch && (
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2">
                      {employeeEnvironment?.icon} {userBranch.branch_name} ·{" "}
                      {userBranch.branch_type} · {tierDisplayName} tier
                    </p>
                  )}
                </div>
                <button
                ...
                >
                  <svg
                    className={`w-3.5 h-3.5 ${loadingTasks ? "animate-spin" : ""}`}
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2.5"
                    viewBox="0 0 24 24"
              ...
              {loadingTasks ? (
                ...
                <div className="py-16 text-center border border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-400 font-semibold mt-1">
                    No pending tasks or duties assigned for your role.
                  </p>
                </div>
              ) : (
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-wider ${badgeClass}`}
                              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_0.7fr] gap-6 text-left">
              ...
              <div className="space-y-6">
                ...
                <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
                  ...
                  <div
                    className="absolute -right-16 -top-16 h-48 w-48 rounded-full blur-[60px] opacity-20 transition-all duration-700"
                    style={{ background: config.accent }}
                  />
                  <div
                    className="absolute -left-12 -bottom-12 h-36 w-36 rounded-full blur-[50px] opacity-15"
                    style={{ background: config.accent }}
                  />
                  ...
                  <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="relative">
                        <div
                          className="h-16 w-16 rounded-[1.25rem] text-white text-xl font-black grid place-items-center shadow-[0_12px_28px_rgba(0,0,0,0.12)] transition-all duration-300 hover:scale-[1.05]"
                          style={{
                            background: `linear-gradient(135deg, ${config.accent} 0%, #1e293b 100%)`,
                          }}
                        >
                          {String(userDisplayName || "M")
                            .trim()
                    ...
                    <div className="flex items-center gap-2.5 sm:self-start">
                      <span
                        className="rounded-full border px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.22em] shadow-sm select-none"
                        style={{
                      ...
                      <button
                        ...
                        className="rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-700 px-4 py-1.5 text-xs font-black uppercase tracking-wider transition-all shadow-sm cursor-pointer select-none"
                      >
                        Edit
                      </button>
                    </div>
                    ...
                    <p className="max-w-md">
                      {isEmployee ? (
                        <>
                          ...
                          at{" "}
                          <strong>
                            {userBranch?.branch_name || "your branch"}
                          </strong>
                          . You can complete assigned tasks and access{" "}
                          {employeeEnvironment?.primaryTab === "billing"
                            ? "POS checkout"
                            : "inventory desk"}{" "}
                          tools for your location.
                        </>
                      ) : isManager ? (
                        <>
                          ✨{" "}
                          ...
                          . You manage staff, assign tasks, and oversee
                          operations for your assigned branch.
                        </>
                      ) : (
                        <>
                          ✨ {config.profile.role}. You possess full operations
                          permissions, ledger scopes, and forecasting triggers
                          for your enterprise.
                        </>
                      )}
                    </p>
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)]">
                  <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-4">
                    <div>
                      ...
                      <h3 className="text-lg font-black text-slate-900 mt-1">
                        Active Registered Branch Networks
                      </h3>
                    </div>
                  ...
                  {!firstLoadDone && branchesList.length === 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {[1, 2].map((n) => (
                        <div
                          key={n}
                          className="animate-pulse bg-slate-50 border border-slate-100 rounded-2xl p-5 h-44 flex flex-col justify-between"
                        >
                          <div>
                    ...
                    <div className="py-8 text-center bg-slate-50/50 border border-dashed border-slate-200 rounded-2xl p-6">
                      ...
                      <span className="block text-sm font-black text-slate-800">
                        No active branch nodes registered in MongoDB yet.
                      </span>
                      <p className="text-xs text-slate-500 font-semibold max-w-sm mx-auto mt-1 leading-normal">
                        Your enterprise branch network has no database mappings.
                        Use the **Growth Console** to initialize your first
                        node!
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                      {branchesList
                        ...
                        .map((branch) => {
                          const isExpanded =
                            !!expandedBranches[branch._id || branch.branch_id];
                          return (
                            <div
                              ...
                              <div>
                                <div className="flex justify-between items-center gap-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span
                                      className={`text-[10px] text-slate-500 transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}
                                    >
                                      ▼
                                    </span>
                                  </div>
                                ...
                                <div className="mt-3.5 flex items-start gap-3">
                                  <div className="w-10 h-10 rounded-xl bg-white border border-slate-200/80 shadow-sm flex items-center justify-center text-xl shrink-0">
                                    {branch.branch_type === "Warehouse"
                                      ? "🏭"
                                      : branch.branch_type === "Franchise"
                                        ? "🤝"
                                        : branch.branch_type === "Depot"
                                          ? "📦"
                                          : "🏪"}
                                  </div>
                                  <div className="min-w-0">
                              ...
                              {!isExpanded && (
                                <div className="grid grid-cols-2 gap-2 mt-4 pt-3.5 border-t border-slate-100 text-left">
                                  <div>
                                    ...
                                    <span className="block text-[10px] font-black text-slate-700 mt-0.5 truncate">
                                      {branch.manager_name}
                                    </span>
                                  </div>
                                  <div>
                                    <span className="block text-[8px] font-black uppercase tracking-wider text-slate-400 text-right">
                                      Employees
                                    </span>
                                    <span className="block text-[10px] font-black text-slate-700 mt-0.5 text-right">
                                      {branch.employee_count} assigned
                                    </span>
                                  </div>
                                </div>
                              )}
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-100 text-left space-y-3.5">
                                  <div className="grid grid-cols-1 gap-y-3.5 text-xs leading-normal">
                                    <div>
                                      ...
                                      <span className="block text-[10.5px] font-bold text-slate-800 leading-relaxed mt-0.5">
                                        {branch.address}
                                      </span>
                                      <span className="block text-[10.5px] font-bold text-slate-800 leading-relaxed mt-0.5">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                      ...
                                      <div>
                                        ...
                                        <span className="block text-[10px] font-bold text-slate-800 mt-1">
                                          {branch.opening_date ||
                                            "Not Specified"}
                                        </span>
                                      </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                      <div>
                                        ...
                                        <span className="block text-[10px] font-black text-slate-800 mt-0.5 truncate">
                                          {branch.manager_name}
                                        </span>
                                      ...
                                      <div>
                                        ...
                                        <span className="block text-[10px] font-black text-slate-800 mt-0.5">
                                          {branch.employee_count} assigned
                                          member
                                    ...
                                    {branch.branch_type !== "Warehouse" &&
                                      branch.gstin && (
                                        <div>
                                          ...
                                          <span className="inline-block text-[9.5px] font-mono font-black uppercase tracking-wider text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md mt-0.5">
                                            {branch.gstin}
                                          </span>
                                        </div>
                                      )}
              ...
              <div className="space-y-6">
                ...
                {isOwner && (
                  <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)] space-y-4">
                    ...
                    <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
                      <span className="text-[9px] font-black uppercase tracking-[0.22em] text-emerald-700">
                        Enterprise Expansion
                      </span>
                      <h4 className="text-base font-black text-slate-950 mt-1">
                        Scale Branch Networks
                      </h4>
                      <p className="text-xs font-semibold text-slate-600 leading-normal mt-1">
                        Register a new branch node inside MongoDB. It will
                        instantly initialize stock records conforming to the
                        latest Complete Inventory Schema.
                      </p>
                      <button
                        onClick={() => {
                          setModalStep(1);
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)] space-y-4">
                  ...
                  <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-[11px] font-mono text-slate-300 space-y-2.5">
                    <div className="flex justify-between items-center py-1 border-b border-slate-800">
                      <span>SECURE_ID:</span>
                      <span className="text-slate-100 font-bold select-all truncate max-w-[140px]">
                        {userSession?.user?.id ||
                    ...
                    <div className="flex justify-between items-center py-1">
                      ...
                      <span className="text-sky-400 truncate">
                        {tierDisplayName.toUpperCase()}_DESK
                      </span>
                    </div>
                  </div>
                ...
                {isOwner && (
                  <div className="bg-white border border-rose-200 rounded-3xl p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05),0_10px_40px_rgba(0,0,0,0.02)] space-y-4">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-[0.24em] text-rose-400">
                        Danger Zone
                      </span>
                      <h3 className="text-base font-black text-slate-900 mt-0.5">
                        Delete Account
                    ...
                    <div className="rounded-2xl border border-rose-100 bg-rose-50/60 p-4">
                      <p className="text-xs font-semibold text-slate-600 leading-relaxed">
                        Permanently remove your account, business profile, all
                        branches, and associated inventory data from Inventra.{" "}
                  ...
                  {accountDeleted ? (
                    <>
                      <div className="text-center mb-6">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.24em] text-amber-500">
                          Account Deactivated
                        </span>
                        ...
                        <p className="text-xs font-semibold text-slate-500 mt-2 leading-relaxed max-w-sm mx-auto">
                          Your data is safely preserved for{" "}
                          <strong className="text-slate-700">30 days</strong>.
                          What would you like to do?
                        </p>
                      </div>

                      {deleteError && (
                        <p className="text-rose-600 text-xs font-bold mb-4 text-center leading-snug">
                          {deleteError}
                        </p>
                      )}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="block text-xs font-semibold text-emerald-700 mt-0.5 leading-snug">
                              Restore everything — all data, branches, and staff
                              access — instantly.
                            </span>
                          </div>
                          <span className="ml-auto text-emerald-600 font-black text-lg shrink-0">
                            →
                          </span>
                        </div>
                      </button>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="block text-xs font-semibold text-slate-500 mt-0.5 leading-snug">
                              Begin fresh with a brand new profile and business
                              setup.
                            </span>
                          </div>
                          <span className="ml-auto text-slate-400 font-black text-lg shrink-0">
                            →
                          </span>
                        </div>
                      </button>
                      ...
                      <p className="text-center text-[10px] font-semibold text-slate-400 mt-4">
                        Recovery window closes in 30 days. After that, data is
                        permanently purged.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="flex items-start gap-4 mb-5">
                        ...
                        <div>
                          <span className="text-[10px] font-black uppercase tracking-[0.24em] text-rose-500">
                            Danger Zone
                          </span>
                          <h3 className="text-xl font-black text-slate-950 mt-0.5 leading-tight">
                            Delete Account
                          </h3>
                          <p className="text-xs font-semibold text-slate-500 mt-1.5 leading-relaxed">
                            ...
                            — data is kept for 30 days so you can recover it.
                            Staff will be locked out but can re-register.
                          </p>
                        </div>
                      </div>
                      ...
                      <label className="block mb-4">
                        ...
                        />
                        {deleteError && (
                          <p className="text-rose-600 text-xs font-bold mt-2 leading-snug">
                            {deleteError}
                          </p>
                        )}
                      </label>
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          disabled={
                            deletingAccount || !deleteConfirmEmail.trim()
                          }
          ...
          {showAddBranchModal &&
            (() => {
              ...
              const ErrMsg = ({ field }) =>
                modalErrors[field] ? (
                  <p className="text-rose-600 text-xs font-bold mt-1.5 leading-none">
                    {modalErrors[field]}
                  </p>
                ) : null;
              ...
              const filteredStates = INDIAN_STATES.filter((stateName) =>
                stateName
                  .toLowerCase()
                  .includes(modalDropdownSearch.toLowerCase()),
              );
              ...
              const validateModalStep = (step) => {
                ...
                if (step === 1) {
                  if (!modalForm.branch_name.trim())
                    errors.branch_name = `${currentLabels.nameLabel} is required.`;
                  if (!modalForm.branch_code.trim())
                    errors.branch_code = `${currentLabels.codeLabel} is required.`;
                  if (!modalForm.branch_type?.trim())
                    errors.branch_type = "Branch type is required.";
                }

                if (step === 2) {
                  if (!modalForm.address.trim())
                    errors.address = `${currentLabels.addressLabel} is required.`;
                  if (!modalForm.city.trim())
                    errors.city = `${currentLabels.cityLabel} is required.`;
                  if (!modalForm.state.trim())
                    errors.state = "State is required.";
                  if (!modalForm.country.trim())
                    errors.country = "Country is required.";
                  if (!modalForm.pincode.trim())
                    errors.pincode = `${currentLabels.pincodeLabel} is required.`;
                }

                if (step === 3) {
                  if (!modalForm.manager_name.trim())
                    errors.manager_name = `${currentLabels.managerLabel} is required.`;
                  if (!modalForm.phone_number.trim()) {
                    errors.phone = `${currentLabels.phoneLabel} is required.`;
                  } else if (
                    modalForm.phone_number.replace(/\D/g, "").length < 7
                  ) {
                      ...
                      <div className="flex items-start justify-between w-full max-w-[480px] mx-auto">
                        {STEPS.map((label, i) => (
                          <React.Fragment key={label}>
                            ...
                            />
                            {i < STEPS.length - 1 && (
                              <StepConnector done={modalStep > i + 1} />
                            )}
                          </React.Fragment>
                        ))}
                      </div>
                                  ...
                                  {BRANCH_TYPES.map((t) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span
                                          className={`text-[9px] font-semibold mt-0.5 leading-none ${isSelected ? "text-slate-300" : "text-slate-400"}`}
                                        >
                                          {t.desc}
                                        </span>
                                      </button>
                          ...
                          <div className="grid grid-cols-12 gap-2">
                            ...
                            <div className="col-span-12">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setModalFormField("address", e.target.value)
                                }
                            ...
                            <div className="col-span-12 sm:col-span-6 relative custom-dropdown-container">
                              ...
                              >
                                ...
                                <svg
                                  className={`w-4 h-4 text-slate-500 transition-transform duration-200 ${modalDropdownOpen ? "rotate-180" : ""}`}
                                  fill="none"
                                  stroke="currentColor"
                                  strokeWidth="2.5"
                                  viewBox="0 0 24 24"
                                ...
                                <div className="absolute left-0 right-0 z-50 mt-1.5 max-h-56 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-2xl p-1.5 flex flex-col gap-1.5">
                                  <div className="px-1.5 py-1 sticky top-0 bg-white z-10 border-b border-slate-100">
                                    <input
                                      ...
                                      onChange={(e) =>
                                        setModalDropdownSearch(e.target.value)
                                      }
                                  ...
                                  <div className="overflow-y-auto max-h-40 space-y-0.5">
                                    {filteredStates.length === 0 ? (
                                      <div className="text-[11px] text-slate-400 font-bold py-2.5 text-center">
                                        No states found
                                      </div>
                                    ) : (
                            ...
                            <div className="col-span-12 sm:col-span-6 relative custom-country-container">
                              ...
                              >
                                <span className="text-slate-900 font-bold flex items-center gap-1.5">
                                  <span>
                                    {COUNTRIES.find(
                                      (c) => c.name === modalForm.country,
                                    )?.flag || "🇮🇳"}
                                  </span>
                                  <span>{modalForm.country}</span>
                                </span>
                                <svg
                                  className={`w-4 h-4 text-slate-500 transition-transform duration-200 ${modalCountryDropdownOpen ? "rotate-180" : ""}`}
                                  fill="none"
                                  stroke="currentColor"
                                  strokeWidth="2.5"
                                  viewBox="0 0 24 24"
                            ...
                            <div className="col-span-12 sm:col-span-6">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setModalFormField("pincode", e.target.value)
                                }
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                                ...
                                <div className="flex relative">
                                  <div className="absolute left-0 top-0 bottom-0 flex items-center z-10">
                                    ...
                                    >
                                      ...
                                      <svg
                                        className={`w-2.5 h-2.5 text-slate-500 transition-transform ${modalPhonePrefixOpen ? "rotate-180" : ""}`}
                                        fill="none"
                                        stroke="currentColor"
                                        strokeWidth="2.5"
                                        viewBox="0 0 24 24"
                            ...
                            <div className="md:col-span-6 flex flex-col gap-2">
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <input
                                    type="number"
                                    min={0}
                                    value={modalForm.employee_count}
                                    onChange={(e) =>
                                      setModalFormField(
                                        "employee_count",
                                        e.target.value,
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    className={
                                      inp("opening_date") + " cursor-pointer"
                                    }
                                  />
                                </div>
                              </div>
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  placeholder={labels.placeholderHours}
                                  value={modalForm.working_hours}
                                  onChange={(e) =>
                                    setModalFormField(
                                      "working_hours",
                                      e.target.value,
                                  ...
                                  className={inp("working_hours")}
                                />
                                <div className="flex flex-wrap gap-1 mt-1.5">
                                  {WORKING_HOURS_PRESETS.map((h) => {
                                    const isSelected =
                                      modalForm.working_hours === h;
                                    return (
                                      <button
                                        key={h}
                                        type="button"
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 bg-slate-950 rounded-2xl p-4 shadow-xl border border-slate-900 text-[13px] leading-tight text-white text-left">
                            <div className="space-y-3 md:border-r md:border-slate-800 md:pr-4">
                              <div className="flex items-center gap-2.5">
                                <div className="w-9 h-9 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-lg">
                                  {BRANCH_TYPES.find(
                                    (t) => t.value === modalForm.branch_type,
                                  )?.icon || "🏪"}
                                </div>
                                <div>
                                  <h5 className="text-white font-black text-sm leading-none">
                                    {modalForm.branch_name ||
                                      `New ${labels.titleLabel}`}
                                  </h5>
                                  <span className="text-emerald-400 text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 px-2 py-0.5 rounded-full inline-block mt-1">
                                    {modalForm.branch_code || "CODE"}
                                  </span>
                                </div>
                              </div>
                              <div className="pt-1">
                                <span className="text-slate-400 text-[9px] font-black uppercase tracking-wider block mb-1">
                                  {labels.addressSectionLabel}
                                </span>
                                <span className="text-slate-300 text-xs font-semibold leading-relaxed block">
                                  {modalForm.address}
                                </span>
                                <span className="text-white text-xs font-bold block mt-1">
                                  {modalForm.city}, {modalForm.state},{" "}
                                  {modalForm.country} – {modalForm.pincode}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="space-y-3 md:border-r md:border-slate-800 md:px-4">
                              <div>
                                <span className="text-slate-400 text-[9px] font-black uppercase tracking-wider block mb-1">
                                  {labels.managerSectionLabel}
                                </span>
                                <span className="text-white text-sm font-bold block">
                                  {modalForm.manager_name}
                                </span>
                              ...
                              {modalForm.branch_type !== "Warehouse" &&
                                modalForm.gstin && (
                                  <div>
                                    <span className="text-slate-400 text-[9px] font-black uppercase tracking-wider block mb-1">
                                      {labels.gstSectionLabel}
                                    </span>
                                    <span className="text-emerald-400 font-mono text-[10px] font-black uppercase tracking-widest">
                                      {modalForm.gstin}
                                    </span>
                                  </div>
                                )}
                            ...
                            <div className="space-y-3 md:pl-4">
                              <div>
                                <span className="text-slate-400 text-[9px] font-black uppercase tracking-wider block mb-1">
                                  {labels.hoursSectionLabel}
                                </span>
                                <span className="text-white text-sm font-bold block">
                                  {modalForm.working_hours}
                                </span>
                              </div>
                              <div>
                                <span className="text-slate-400 text-[9px] font-black uppercase tracking-wider block mb-1">
                                  {labels.teamSectionLabel}
                                </span>
                                <span className="text-white text-xs font-bold block">
                                  {modalForm.employee_count} Additional Staff
                                  Member
                                  {modalForm.employee_count !== 1 ? "s" : ""}
                                </span>
                                {modalForm.opening_date && (
                                  <span className="text-slate-400 text-[10px] font-semibold block mt-0.5">
                                    {labels.openingDateLabel}:{" "}
                                    {modalForm.opening_date}
                                  </span>
                                )}
                              </div>
...
function BranchManagerOverview({
  products,
  salesCount,
  salesRevenue,
  salesRevenueNote,
  ...
  const statCards = [
    ...
    {
      label: "Revenue",
      value: `₹${(salesRevenue || 0).toLocaleString("en-IN")}`,
      icon: "💰",
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {statCards.map((card) => (
          ...
          >
            ...
            <div className="text-[10px] font-black uppercase tracking-wider mt-1 opacity-80">
              {card.label}
            </div>
          </div>
        ))}
