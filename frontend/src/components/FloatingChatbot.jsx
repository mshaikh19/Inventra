...
export default function FloatingChatbot({ activeTier }) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const res = await fetch(`${apiBase}/api/v1/chatbot/history`, {
          headers: {
  ...
  const persistMessage = async (sender, text, payload = null) => {
    const token = localStorage.getItem("inventra_token") || sessionStorage.getItem("inventra_token");
    if (!token) return;
    try {
      const apiBase = import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000";
      await fetch(`${apiBase}/api/v1/chatbot/history/message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
        },
        body: JSON.stringify({ sender, text, payload })
      });
    } catch (err) {
  ...
  const handleClearHistory = async () => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000";
      await fetch(`${apiBase}/api/v1/chatbot/history`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    persistMessage("user", textToSend);
    setInputValue("");
    setIsTyping(true);
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/v1/chatbot/query`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (data.is_fallback) {
        // Run local simulation matching
        const localResponse = handleQuery(textToSend);
        const displayResponse = {
          sender: "ai",
          text: localResponse.text,
          payload: localResponse.payload
        };
        
        // Show fallback indicator once per session if returned
        if (data.text && !messages.some(m => m.text.includes("GEMINI_API_KEY"))) {
          setMessages((prev) => [
            ...prev,
            { sender: "ai", text: data.text },
            displayResponse
          ]);
          persistMessage("ai", data.text);
          persistMessage("ai", displayResponse.text, displayResponse.payload);
        } else {
      ...
      console.warn("Live chatbot connection failed, falling back to local simulation:", e);
      const localResponse = handleQuery(textToSend);
      setMessages((prev) => [
        ...prev,
        { sender: "ai", text: localResponse.text, payload: localResponse.payload }
      ]);
      persistMessage("ai", localResponse.text, localResponse.payload);
    } finally {
      setIsTyping(false);
      playChime("incoming");
    }
